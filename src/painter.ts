...
export default class Painter {
  ...
  globalWidth: Record<string, any> = {};
  globalHeight: Record<string, any> = {};
  style: Record<string, any> = {};
  constructor(ctx: CanvasRenderingContext2D, data: Painting) {
    this.ctx = ctx;
    this.data = data;
    this.globalWidth = {};
    this.globalHeight = {};
  ...
  paint(callback: Function) {
    this.style = {
      width: this.data.width && this.data.width.toPx(),
      height: this.data.height && this.data.height.toPx(),
    };
    this._background();
  ...
  _background() {
    ...
    this.ctx.translate(width / 2, height / 2); // 将 canvas 按原始 x 点的水平方向、原始的 y 点垂直方向进行平移变换，（0，0）画布的中心
    this._doClip(width, height, this.data.borderRadius);
    if (!bg) {
      // 如果未设置背景，则默认使用透明色
      this.ctx.fillStyle = "transparent";
      this.ctx.fillRect(-(width / 2), -(height / 2), width, height);
    } else if ((bg as HTMLImageElement).src) {
      // 背景填充图片
      this.ctx.drawImage(
      ...
      (bg as string).startsWith("#") ||
      (bg as string).startsWith("rgba") ||
      (bg as string).toLowerCase() === "transparent"
    ) {
      // 背景填充颜色
      this.ctx.fillStyle = bg as string;
      this.ctx.fillRect(-(width / 2), -(height / 2), width, height);
    ...
    if (borderRadius) {
      ...
      if (border.length === 4) {
        ...
        r1 =
          ...
          r4 =
            Math.min(
              borderRadius ? borderRadius.toPx(false, minSize) : Infinity,
              width / 2,
              height / 2
            );
      }
    }
    const lineWidth = borderWidth ? borderWidth.toPx(false, minSize) : 0;
   ...
   * 画边框
   */
  _doBorder(view: any, width: any, height: any) {
    if (!view.css) {
      return;
    }
  ...
  _preProcess(view: any, notClip?: any) {
    ...
    switch (view.type) {
      case "text": {
        ...
        const fontWeight = fontWeightArr.includes(view.css.fontWeight)
          ? view.css.fontWeight
          : "normal";
    ...
    if (view.css && view.css.right) {
      if (typeof view.css.right === "string") {
        ...
        x =
          ...
          this.globalWidth[rights[1]] * (rights[2] || 1);
      }
      ...
      if (typeof view.css.left === "string") {
        ...
        x =
          ...
          this.globalWidth[lefts[1]] * (lefts[2] || 1);
      }
    } else {
      x = 0;
    }
      ...
      if (view.css && view.css.top) {
        if (typeof view.css.top === "string") {
          ...
          y =
            ...
            this.globalHeight[tops[1]] * (tops[2] || 1);
        }
      } else {
        y = 0;
      }
    ...
    if (!notClip && view.css && view.css.borderRadius && view.type !== "rect") {
      this._doClip(width, height, view.css.borderRadius, view.css.borderStyle);
    }
    this._doShadow(view);
    if (view.id) {
  ...
  _doPaddings(view: any) {
    const { padding } = view.css ? view.css : { padding: undefined };
    let pd = [0, 0, 0, 0];
    if (padding) {
  ...
  _drawQRCode(view: any) {
    this.ctx.save();
    const { width, height } = this._preProcess(view) as any;
    QR.api.draw(
      view.content,
      this.ctx,
      -width / 2,
  ...
  _fillAbsText(view: any) {
    ...
    for (let j = 0; j < textArray.length; ++j) {
      ...
      const firstLineLength = Math.ceil(
        ((width - textIndent) / width) * (textArray[j].length / linesArray[j])
      );
      let start = 0;
      let alreadyCount = 0;
      ...
      for (let i = 0; i < linesArray[j]; ++i) {
        ...
        while (
          ...
          (width - measuredWith - tabWidth > view.css.fontSize.toPx() ||
